...
class FollowersSuite extends munit.FunSuite with TestKitBase {
  ...
  def await[A](futureValue: Future[A]): A = Await.result(futureValue, 5000.milliseconds)

  test("reframedFlow: chunks containing exactly one message should pass through (2pts)") {
  ...
  test("reframedFlow: chunks containing fragments of messages should be re-assembled (3pts)") {
    val incoming = List(ByteString("f"), ByteString("oo\nb"), ByteString("ar\n"))
    val got = await(Source(incoming).via(reframedFlow).runWith(Sink.seq))
    assertEquals(got, Seq("foo", "bar"))
  }

  test("reframedFlow: reject an input stream that is completed in the middle of a frame (3pts)") {
  ...
  test("outgoingFlow: filter out events that should not be delivered to the given user (3pts)") {
    val server = new Server()
  ...
  test("clientFlow: ensure that event before issuing a Follow is not sent to that follower (6pts)") {
    val server = new Server()
  ...
  test("clientFlow: ensure that users get notified from private messages (6pts)") {
    val server = new Server()
    val follower1 = connectClient(1, server)
    val follower2 = connectClient(2, server)
