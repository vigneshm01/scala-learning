...
def boxBlurKernel(src: Img, x: Int, y: Int, radius: Int): RGBA =
  ...
  var b: Int = 0
  var a: Int = 0
  val minXOffset: Int = clamp(x - radius, 0, src.width - 1)
  val minYOffset: Int = clamp(y - radius, 0, src.height - 1)
  val maxXOffset: Int = clamp(x + radius, 0, src.width - 1)
  val maxYOffset: Int = clamp(y + radius, 0, src.height - 1)
