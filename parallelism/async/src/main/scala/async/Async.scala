  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] =
    if (maxAttempts > 1)
      makeAsyncComputation() recoverWith{
        case _: Throwable => insist(makeAsyncComputation, maxAttempts-1)
      }
  ...
  def futurize(callbackBasedApi: CallbackBasedApi): FutureBasedApi = new FutureBasedApi {
    override def computeIntAsync(): Future[Int] =
    {
      ...
      callbackBasedApi.computeIntAsync{
        case Success(value) => pro.success(value)
        case Failure(exception) => pro.failure(exception)
      }
      pro.future
    }
  }
