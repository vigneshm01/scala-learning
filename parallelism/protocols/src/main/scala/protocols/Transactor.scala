...
object Transactor {
  ...
  private def idle[T](value: T, sessionTimeout: FiniteDuration): Behavior[PrivateCommand[T]] =
      Behaviors.receive { (context, message) =>
        message match {
          case Begin(replyTo) =>
            val session = context.spawnAnonymous(sessionHandler(value, context.self, Set.empty))
            context.watch(session)
            context.scheduleOnce(sessionTimeout, context.self, RolledBack(session))
          ...
          case _: PrivateCommand[T] => Behaviors.same
        }
      }
