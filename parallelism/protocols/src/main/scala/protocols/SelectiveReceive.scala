...
object SelectiveReceive {
  ...
  def apply[T: ClassTag](bufferCapacity: Int, initialBehavior: Behavior[T]): Behavior[T] =
    Behaviors.withStash(bufferCapacity){
      buffer => intercept(bufferCapacity, buffer, Behavior.validateAsInitial(initialBehavior))
    }
    ...
    Behaviors.receive {
      ...
      case (ctx, message) =>
        ...
        if (Behavior.isUnhandled(nextBehavior)) {
          ...
          buffer.unstashAll(SelectiveReceive(bufferSize, canonicalNext))
        }
    }
