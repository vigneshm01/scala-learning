...
object Manipulation extends ManipulationInterface:
  ...
  def makeGrid(temperatures: Iterable[(Location, Temperature)]): GridLocation => Temperature =
    ...
    val fun = (gLoc: GridLocation) => memo.get(gLoc) match
      case None =>
        val res = predictTemperature(temperatures, Location(gLoc.lat, gLoc.lon))
        memo += gLoc -> res // memoize it
        res
      case Some(value) => value // look up memoized value
    fun
