...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences =
    val words = w.toLowerCase.toList
    words.map(l =>(l, words.count(_ == l))).distinct.sortBy(_._1)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    (y foldLeft x.toMap) { case (map, (letter, freq)) =>
      val subtractedFreq = map(letter) - freq
      if subtractedFreq > 0 then map.updated(letter,subtractedFreq)
      else map - letter
    }.toList
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] ={
    if (sentence.isEmpty) List(List())
      ...
      def findWords(occurrences: Occurrences): List[Word] = {
        dictionaryByOccurrences.getOrElse(occurrences, List())
      }
      def recur(occurrences: Occurrences): List[Sentence] = {
        if (occurrences.isEmpty) List(List())
        else {
          val combinationsList = combinations(occurrences)
          val sentencesList = combinationsList.flatMap { combination =>
            val wordsList = findWords(combination)
            if (wordsList.isEmpty) List()
            else {
              val restOccurrences = subtract(occurrences, combination)
              val restSentences = recur(restOccurrences)
              restSentences.map(wordsList.head :: _)
            }
          }
          sentencesList
        }
